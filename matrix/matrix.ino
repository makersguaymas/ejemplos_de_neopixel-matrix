...
Adafruit_NeoMatrix matrix = Adafruit_NeoMatrix(5,8, PIN,NEO_MATRIX_TOP + NEO_MATRIX_LEFT + NEO_MATRIX_COLUMNS ,NEO_GRB + NEO_KHZ800);
...
void loop() {
      ...
      for(int i = 0; i < 8; i++){
        //columnas
        for(int y = 0; y < 8; y++){
            matrix.drawPixel(i, y, gamaAmarilla[i]);
            matrix.show();
        }
      }
      if(maxBright == false){
